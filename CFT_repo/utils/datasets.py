...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            for p_rgb in path_rgb if isinstance(path_rgb, list) else [path_rgb]:
                ...
                if p_rgb.is_dir():  # dir
                    ...
                    with open(p_rgb, 'r', encoding='utf-8') as t:
                        t = t.read().strip().splitlines()
                ...
                for p_ir in path_ir if isinstance(path_ir, list) else [path_ir]:
                    ...
                    if p_ir.is_dir():  # dir
                        ...
                        with open(p_ir, 'r', encoding='utf-8') as t:
                            t = t.read().strip().splitlines()
            ...
            self.img_files_rgb = sorted([x.replace('\\', '/') for x in f_rgb if x.split('.')[-1].lower() in img_formats])
            self.img_files_ir = sorted([x.replace('\\', '/') for x in f_ir if x.split('.')[-1].lower() in img_formats])
        ...
        self.img_files_rgb = [k.replace('\\', '/') for k in cache_rgb.keys()]  # update
        self.label_files_rgb = img2label_paths(self.img_files_rgb)  # update
        if single_cls:
        ...
        self.img_files_ir = [k.replace('\\', '/') for k in cache_ir.keys()]  # update
        self.label_files_ir = img2label_paths(self.img_files_ir)  # update
        if single_cls:
...
def load_image_rgb_ir(self, index):
    ...
    if (img_rgb is None) and (img_ir is None):  # not cached
        ...
        img_rgb = cv2.imdecode(np.fromfile(path_rgb, dtype=np.uint8), cv2.IMREAD_COLOR)
        img_ir = cv2.imdecode(np.fromfile(path_ir, dtype=np.uint8), cv2.IMREAD_COLOR)
